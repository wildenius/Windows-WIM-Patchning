...
- Added output selection with `SWM` as default, plus `WIM` and `Both` modes via `-OutputMode`; the choice now lives inside the single startup menu instead of a separate screen.
