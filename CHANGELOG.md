...
- Added output selection with `SWM` as default, plus `WIM` and `Both` modes via `-OutputMode`; the choice now lives inside the single startup menu instead of a separate screen. The same menu also shows injectable patch rows with release date and package size.
...
- Latest-update discovery now uses explicit package type cache keys: LCU, DotNet, and SafeOS, and caches Microsoft Update Catalog package size metadata for menu display.
- BuildWIM operator flow now gates expensive ISO/download work behind one combined startup menu for update and output-format decisions instead of downloading source media first.
